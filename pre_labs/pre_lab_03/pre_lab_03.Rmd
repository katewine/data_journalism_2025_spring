...
**Answer** It says one or more parsing issues. 
...
**Answer** 1640 problems. It seems that R expected a true/false value, but in its place is a character string in the form of numbers 
...
**Task** Run the following codeblock to glimpse the data. What data type is the "district" field? What data type is the "precinct" field? What data type is the "mail" column? Answer below. **Answer** District is a dbl datatype. Precinct is a chr datatype. Mail is a dbl datatype.
...
**Task** Run the following codeblock to group by mail votes, count the number of precinct results, then sort from highest to lowest on count. How many results are there where mail is NA? What's the implication there? Answer below. **Answer**: There are 402345 results where mail is NA. The implication could be that mail votes were not an option in many precincts.
...
**Answer** There are 12 registrations in January 1900. The first 20 results suggest that perhaps that number is not accurate, because each of the other months only has about 1 voter each. Having 12 voters over the age of 120 vote in a single county seems far fetched.
...
**Answer** I think the deceased and legacy status reason would be worthy exploration that could probably explain why it says 12 people who registered to vote in 1900 would be among recent voter data.
...
**Task** Run the following codeblock to glimpse the data. What data type is the "amount" field? Answer below. **Answer** Amount is a chr data type despite being numbers
...
**Answer** 1. The column names have spaces in them. 
2. Inconsistent capitalization across multiple columns.
The zip field mixes five digit ZIP codes and nine digit ZIP codes, and some of the records include spaces. 
...
**Answer** The function added an x to  1_linenumber.
...
**Task** Run the following codeblock to attempt to add up the amount of all contributions. What does the error say when you run this code? What do you think the error means? Answer below. **Answer**Invalid 'type' of argument. There is a character string when there should be some type of numerical value
...
**Answer** There are two duplicate rows for a donation from a person named Derrick Hamilton.
...
**Task** Run the following codeblock to use distinct() to get rid of duplicate rows. How many rows does the new dataframe have? Answer below. Add a description of what this code does to your reference notebook. **Answer** The dataframe has 13 rows. n_distinct got rid of the duplicates.
...
**Answer** The zipcodes only have five digits
...
**Answer** There are two mispellings of Conowingo. Conowing and Conowingoo
...
**Task** Run the following codeblock to use case_when() to fix misspellings of Conowingo in the "city" field. How many mispellings of Conowingo remain after running this code? Answer below. **Answer** There is still one mispelling
...
**Task** Run the following codeblock to use case_when() to fix misspellings of Conowingo in the "city" field using both the exact match method and the str_detect() method. How many mispellings of Conowingo remain after running this code? Answer below. Add a description of what this code does to your reference notebook. **Answer**There are no more mispellings.
