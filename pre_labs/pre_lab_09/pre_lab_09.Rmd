...
**Answer** Copy the url of the graphic you published here. It will begin with <https://datawrapper.dwcdn.net/>. Don't copy the browser's current url. https://datawrapper.dwcdn.net/oeq9O/1/
...
md_gov_county <- md_gov_county |>
  clean_names()|>
  mutate(
    total_votes = (moore + cox + lashar + wallace+harding + write_ins),
...
**Task** Look for "Hide regions without data" under Appearance, and click the slider icon to enable that feature. You should see a map zoomed into Maryland with some counties in various colors. **Answer** Now the map only shows Maryland at a tilt and broken down by counties with different colors.
...
**Task** Click the "Customize tooltips" button so it expands down. Change {{ fips_code }} to {{ county }} and {{ pctmoore }} to {{ FORMAT(pctmoore, "00.0%")}} **Answer** Now when you hover over the county it shows its name nad a clearer the correct percentage that correpsonds to the county.
...
https://datawrapper.dwcdn.net/rxj2I/1/ 
