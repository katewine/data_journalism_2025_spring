...
**Task** Run the following code and describe the dataframe it outputs. **Answer** The dataframe is a collection of Ben Cardin's press releases
...
**Task** Run the following code and describe what the `unique` function does to the original list of words. **Answer** The unique function shows all the unique or distinct words that show up in the list.
...
**Task** Run the following code and describe the output of using the `unnest_tokens` function. **Answer** This creates a column of every single word the text uses.
...
**Task** Run the following code and describe what the resulting graphic shows. Is it interesting? **Answer** This is not very interesting because it simply shows the most common words like "the" or "and." This is not a surprising or interesting result.
...
**Task** Run the following code and describe the results. Is it more interesting than before? **Answer** This is much more interesting than before. These words might not be the same as any other politician, unlike the previous dataframe.
...
**Task** Run the following code and describe the results. **Answer** This splits up the top words per year, so that you can draw a comparison between the top topics in his press releases, and how his focus has shifted.
...
**Task** Run the following code and describe the results. **Answer** This gives the top two-word phrases for 2022. Amount included and health care were the top two phrases that year.
...
**Task** Run the following code and describe the results. **Answer** This did the same thing as the previous code but for 2024. Here the top phrase is health care and amount awarded.
...
**Task** Run the following code and describe the differences between the two results. Which one might be more useful for the press release data? **Answer**  Stemming only changed drives and driving to drive while lemmatizing changed all tenses of drive to drive, and kept driver as driver. I think Lemmatization would be more useful because the tense of the word does not matter as much as the meaning. In all cases they are verbs. However, it recognized driver as its own word because it is a noun, and not a conjugation of drive, which is good,
...
**Task** Run the following code and describe the differences between the two results. Which one is more useful? **Answer** In the lemmatized version, fund, community and support were the top three words. In the normal version, Baltimore, support and funding were the top words. I think the lematized version is better because each tense of fund is better represented and counted as one.
...
**Task** Run the following code and describe the results. Do any of the sentiments seem incorrect or counter-intuitive? **Answer** This assigns a positive or negative sentiment to each word. It could be counter intuitive to assign the words sentiments out of context. For example, it assigns the word cancer a negative sentiment, but maybe the press release was about a positive step towards better treatment or more funding for research. Also relief is deemed positive, but maybe the press release is about a lack of relief, which would not be a positive context.
