...
**QUESTION FOR RYAN** Hi, we were unable to find the contribution data for the Rush Baker campaign through the website, so instead of loading all seven candidates we only have six.
...
* **Analysis summary**: First, I would like to complain about finding the data on the Maryland Campaign contribution database. That website is hell. OK. We were able to confirm the finding. out of Wes Moore's $4,127,418 in campaign fudning, according to his campaigns 2021 Annual report, $2387822.72	of that came from out of state. Less than $2 million was actually from maryland contributors. In our code that differentiates Maryland from Out of State contributions, there is an NA, and I'm not sure why. It must be from the mutate(case_when...) code, but I consulted chatgpt to get the state out of the address. Even with that issue though, we were still able to re-engineer this first sentence.
